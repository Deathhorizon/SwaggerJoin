...
//            }
        ...
        private async Task<Dictionary<string, SwaggerDocument>> GetDownstreamSwaggerDocuments(Configuration ocelotConfiguration)
        {
            ...
            foreach (var host in downstreamHosts)
            {
                if (dic.ContainsKey(host.ToUrl()))
                 ...
                 var swaggerDocumentUrl = $"http://{host.ToUrl()}/swagger/v1/swagger.json";
                //var swaggerDocumentUrl = string.Format(settings.DownstreamSwaggers, host.ToUrl());
